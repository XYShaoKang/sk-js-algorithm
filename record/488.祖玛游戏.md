 ...
 * @returns 返回消除后的字符串
 */
const clear = (board: string, t = board.replace(/(\w)\1{2,}/, '')): string =>
  t === board ? board : clear(t)
...
function findMinStep(board: string, hand: string): number {
  ...
  while (index < queue.length) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      for (let j = 0; j < n; j++) {
        if ((board[j] !== s && board[j] !== board[j - 1]) || board[j - 1] === s)
          continue
...
test.each([
  ...
  'input: board = $input.board, hand = $input.hand',
  ({ input: { board, hand }, output }) => {
